...
|参与者|图书管理员（主要参与者）、读者（次要参与者）|
|前置条件|图书管理员已被识别和授权|
|后置条件|存储结束记录，更新库存数量，所借图书状态为已归还入库状态|
|主事件流|1.图书管理员将读者借书卡提供给系统 2.系统验证读者身份和借书条件 3.图书管理员将读者所要归还图书输入系统 4.系统记录还书信息，并修改图书的状态和此种书的可借数量 5.系统累计加读者的还书数量，减少读者的借书数量 6.重复3~5，直到图书管理员确认全部图书等级完毕 7.系统打印还书清单，交易完成|
|备注|图书馆开架借阅，读者找到书后办理借书手续，因此还书不需要验证库存，而且每本书都是可识别的|
...
![usecase](https://github.com/lfd1109550635/is_analysis/blob/master/test2/信息8.png)
